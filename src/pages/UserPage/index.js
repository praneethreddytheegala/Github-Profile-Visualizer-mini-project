    ...
    try {
      ...
      if (!response) {
        setError({
          active: true,
          status: 404,
          message: 'No repo found for this user!',
        })
      }
    ...
    try {
      ...
      if (!response) {
        setError({
          active: true,
          status: 404,
          message: 'Could not fetch rate limit!',
        })
      }
      ...
      if (rateLimit.remaining === 0) {
        setError({
          active: true,
          status: 403,
          message: "You've hit the rate litmit!",
        })
      }
      ...
      {error && error.active ? (
        ...
        <StyledContainer>
          ...
          <Head
            title={`${
              username
                ? `${username} | GitHub Profile Visualizer`
                : 'GitHub Profile Visualizer'
            }`}
          />
          <StyledContent>
            <StyledSectionHeader error={error}>
              Hello, {username}. Here are your GitHub profile statistics.
            </StyledSectionHeader>
            {langData && repoData && (
              <Charts
                langData={langData}
